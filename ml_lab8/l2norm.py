...
def compute_cost(X,y,theta,lambda_reg):
    ...
    cost=(1/(2*m))*np.sum((prediction-y)**2) + (lambda_reg/(2*m)) * np.sum(theta**2)
    return cost
def ridge_regressionL2(X,y,alpha=0.01,lambda_reg=0.1,num_iters=1000):
    m=len(y)
    m,n=X.shape
    ...
    for i in range(num_iters):
        ...
        gradient= (1/m)* np.dot(X.T,(predictions-y))+(lambda_reg * theta)
...
def main():
    ...
    #
    theta,costs=ridge_regressionL2(X_train,y_train,alpha=0.01,lambda_reg=0.01,num_iters=1000)
